...
import { ArrowUpRight, Play, CheckCircle, Zap, Package, Activity, Bot, TrendingUp } from 'lucide-react'
...
// ─── Hero hub illustration with a clean legend grid below (no overlap ever) ──
function HubVisual() {
  const nodes = [
    { text: 'Product Studio',   model: 'qwen-omni-turbo',   color: '#8b5cf6' },
    { text: 'Reasoning Engine', model: 'qwen-max',          color: '#3b82f6' },
    { text: 'Live Activity',    model: 'text-embedding-v4', color: '#10b981' },
    { text: 'Approval Queue',   model: 'human-in-the-loop', color: '#f97316' },
  ]
  ...
  return (
    <div className="relative w-full max-w-xl mx-auto">
      ...
      {/* Legend grid — deliberately placed BELOW the image, never overlapping it,
          so this is stable at every viewport width with zero risk of clipping. */}
      <motion.div
      ...
      >
        {nodes.map(n => (
          <div
            key={n.text}
            className="bento-card px-3.5 py-2.5 flex items-center gap-2 min-w-0"
          >
            <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: n.color, boxShadow: `0 0 8px ${n.color}` }} />
...
// ─── Bento stat card with SVG arc progress ring ───────────────────────────────
...
export default function Landing() {
  return (
    <div className="relative min-h-screen bg-[#070707] overflow-x-hidden dot-grid">
      ...
      {/* ── Watch it reason (relocated terminal execution preview) ──────────── */}
