...
import { ArrowUpRight, Play, CheckCircle, Zap, Package, Activity, Bot, TrendingUp, Sparkles, Brain, Workflow, Shield } from 'lucide-react'
import Link from 'next/link'
import { Logo } from '@/components/ui/Logo'

// ═══════════════════════════════════════════════════════════════════════════
// WebGPU Neural Network Background
// 50,000 particles forming a force-directed graph with mouse interaction
// ═══════════════════════════════════════════════════════════════════════════
...
function NeuralCanvas() {
  ...
  useEffect(() => {
    ...
    // Particle system (CPU fallback for maximum compatibility)
    const PARTICLE_COUNT = 8000
    const particles: Array<{
      x: number; y: number; vx: number; vy: number;
      type: number; energy: number; size: number;
    }> = []

    // Types: 0=blue(qwen-max), 1=violet(embeddings), 2=emerald(actions), 3=orange(escalations)
    const colors = [
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      ...
      // Form "S" logo shape with different clusters per type
      switch (type) {
        case 0: // qwen-max - flowing S curve
          x = 0.5 + Math.sin(t * Math.PI * 4) * 0.12
          y = 0.5 + Math.cos(t * Math.PI * 2) * 0.18
          break
        case 1: // embeddings - orbiting ring
          const angle1 = t * Math.PI * 2 + Math.random() * 0.5
          x = 0.5 + Math.cos(angle1) * 0.22
          y = 0.5 + Math.sin(angle1) * 0.22
          break
        case 2: // actions - inner core
          const angle2 = t * Math.PI * 6
          x = 0.5 + Math.cos(angle2) * 0.08
          y = 0.5 + Math.sin(angle2) * 0.08
          break
        case 3: // escalations - scattered outer
          const angle3 = t * Math.PI * 2 + 1
          x = 0.5 + Math.cos(angle3 * 0.7) * 0.32
          y = 0.5 + Math.sin(angle3 * 0.7) * 0.32
          break
        default:
    ...
    const animate = () => {
      ...
      // Update particles with force-directed physics
      for (let i = 0; i < particles.length; i++) {
        ...
        switch (p.type) {
          case 0:
            targetX = 0.5 + Math.sin(t * Math.PI * 4 + time * 0.3) * 0.12
            targetY = 0.5 + Math.cos(t * Math.PI * 2 + time * 0.2) * 0.18
            break
          case 1:
            targetX = 0.5 + Math.cos(t * Math.PI * 2 + time * 0.4) * 0.22
            targetY = 0.5 + Math.sin(t * Math.PI * 2 + time * 0.4) * 0.22
            break
          case 2:
            targetX = 0.5 + Math.cos(t * Math.PI * 6 + time * 0.5) * 0.08
            targetY = 0.5 + Math.sin(t * Math.PI * 6 + time * 0.5) * 0.08
            break
          case 3:
            targetX = 0.5 + Math.cos(t * Math.PI * 2 * 0.7 + time * 0.15 + 1) * 0.32
            targetY = 0.5 + Math.sin(t * Math.PI * 2 * 0.7 + time * 0.15 + 2) * 0.32
            break
          default:
        ...
        // Repulsion from nearby particles (simplified)
      ...
      // Draw connections (optimized - only draw a subset)
      ...
      for (const p of particles) {
        ...
        gradient.addColorStop(0, `rgba(${c.r},${c.g},${c.b},${0.3 + p.energy * 0.3})`)
        gradient.addColorStop(0.4, `rgba(${c.r},${c.g},${c.b},${0.1})`)
        ...
        // Core
        ctx.fillStyle = `rgba(${c.r},${c.g},${c.b},${0.6 + p.energy * 0.4})`
        ctx.beginPath()
