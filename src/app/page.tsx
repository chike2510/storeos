...
// ─── Hero hub illustration with floating feature labels ──────────────────────
function HubVisual() {
  const labels = [
    { text: 'Product Studio',   model: 'qwen-omni-turbo',   top: '26%', left: '6%',  color: '#8b5cf6', delay: 0.5 },
    { text: 'Reasoning Engine', model: 'qwen-max',          top: '22%', right: '2%', color: '#3b82f6', delay: 0.65 },
    { text: 'Live Activity',    model: 'text-embedding-v4', bottom: '20%', left: '2%', color: '#10b981', delay: 0.8 },
    { text: 'Approval Queue',   model: 'human-in-the-loop', bottom: '16%', right: '4%', color: '#f97316', delay: 0.95 },
  ]
    ...
    <div className="relative w-full max-w-xl mx-auto">
      ...
      {labels.map((l, i) => (
        <motion.div
          ...
          transition={{ delay: l.delay, type: 'spring', stiffness: 260, damping: 26 }}
          style={{ top: l.top, left: l.left, right: l.right, bottom: l.bottom }}
          className="absolute glass rounded-full px-3 py-1.5 border flex items-center gap-1.5 whitespace-nowrap"
        >
          <span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ backgroundColor: l.color }} />
          <span className="text-[11px] font-display font-medium text-white/75">{l.text}</span>
          <span className="text-[9px] font-mono text-white/25 hidden sm:inline">{l.model}</span>
        </motion.div>
      ))}
    </div>
  )
